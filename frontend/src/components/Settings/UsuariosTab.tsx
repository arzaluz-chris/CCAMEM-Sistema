...
export default function UsuariosTab() {
  ...
  const loadUnidades = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/catalogos/unidades`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleSubmitPassword = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(
        `${process.env.REACT_APP_API_URL}/usuarios/${selectedUser?.id}/cambiar-password`,
        {
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 2 }}>
            ...
            <FormControl fullWidth>
              ...
              <Select
                value={formData.unidadAdministrativaId}
                label="Unidad Administrativa"
                onChange={(e) =>
                  setFormData({ ...formData, unidadAdministrativaId: e.target.value })
                }
              >
      ...
      <Dialog open={openPasswordDialog} onClose={handleClosePasswordDialog} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Alert severity="info" sx={{ mb: 2, mt: 2 }}>
            Estableciendo nueva contraseña para: <strong>{selectedUser?.username}</strong>
          </Alert>
          <Stack spacing={2}>
            <TextField
              ...
              onChange={(e) =>
                setPasswordData({ ...passwordData, passwordNuevo: e.target.value })
              }
              fullWidth
              required
              helperText="Mínimo 6 caracteres"
